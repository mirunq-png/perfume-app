...
export default function Filter() {
  ...
  const [filterType, setFilterType] = useState('note'); // 'note' or 'season'
  ...
  const filterData = async () => {
    console.log('filterData called', { filterType, noteInput, selectedSeason });
  ...
  const handleKeyPress = (e) => {
    if (e.key === 'Enter' && filterType === 'note') {
      filterData();
    }
  };
      ...
      <div className="filter-content">
        <div className="search-group">
          ...
          <div className="filter-type-toggle">
            <button 
              className={`toggle-btn ${filterType === 'note' ? 'active' : ''}`}
              onClick={() => { setFilterType('note'); setSearched(false); setPerfumes([]); }}
            >
              Search by note
            </button>
            <button 
              className={`toggle-btn ${filterType === 'season' ? 'active' : ''}`}
              onClick={() => { setFilterType('season'); setSearched(false); setPerfumes([]); }}
            >
              Search by season
            </button>
          </div>
        ...
        <div className="results-container">
          ...
          {!loading && searched && perfumes.length === 0 && !error && (
            <p className="status-message">
              No perfumes found for this search.
            </p>
          )}
          ...
          {!loading && perfumes.length > 0 && (
            <>
              <h3>
                Found {perfumes.length} matches 
                {filterType === 'note' ? ` containing '${noteInput}'` : ` for ${formatText(selectedSeason)}`}
                :
              </h3>
              <ul className="results-list">
                {perfumes.map((p, index) => (
                  <motion.li
                    ...
                    onClick={() => navigate(`/layer/${p.id}`)}
                    style={{ cursor: 'pointer' }}
                  >
                    <div className="result-card-header">
                      <span className="result-name">
                        [{index + 1}] {formatText(p.brand.name)} - {formatText(p.name)}
                      </span>
                    </div>
